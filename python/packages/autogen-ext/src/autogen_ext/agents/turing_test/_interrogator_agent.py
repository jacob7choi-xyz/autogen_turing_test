        ...
        **kwargs,
        ...
        if system_message is None:
            system_message = self._create_default_system_message(
                conversation_style, question_categories
            )
        ...
        self.question_categories = (
            question_categories or self._default_question_categories()
        )
        self.current_round = 0
        self.conversation_log: List[Dict[str, Any]] = []
        self.start_time: Optional[datetime] = None

    def _create_default_system_message(
        self, style: str, categories: Optional[List[str]]
    ) -> str:
        """Create a default system message for the interrogator."""

        base_prompt = """You are an experienced interrogator conducting a Turing Test.
Your goal is to determine whether the participant you're conversing with is
human or AI through strategic questioning.

CORE OBJECTIVES:
1. Ask thoughtful, probing questions to reveal human vs AI characteristics
...
4. Detect AI patterns like overly structured responses or lack of personal
   experience
            ...
            self.conversation_log.append(
                {
                    "timestamp": datetime.now().isoformat(),
                    "round": self.current_round,
                    "interrogator_question": msg_content,
...
Provide your verdict as either "HUMAN" or "AI" followed by reasoning."""
        ...
        final_message = TextMessage(content=content, source=self.name)
...
KEY EXCHANGES:"""
        ...
        for entry in self.conversation_log:
            if "participant_message" in entry:
                msg = entry["participant_message"][:100]
                summary += f"\nRound {entry['round']}: {msg}..."
            if "interrogator_question" in entry:
                question = entry["interrogator_question"][:100]
                summary += f"\nQ{entry['round']}: {question}..."
    ...
    def reset_conversation(self) -> None:
        ...
        logger.info("Interrogator %s reset for new conversation", self.name)
...
def create_interrogator(
    ...
    **kwargs,
    ...
    base_config = type_configs.get(interrogator_type, type_configs["investigative"])
    # Merge configurations with kwargs taking precedence
    merged_config = dict(base_config)
    merged_config.update(kwargs)

    return InterrogatorAgent(name=name, model_client=model_client, **merged_config)
